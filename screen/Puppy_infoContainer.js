...
export default function Puppy_infoContainer(){
    const [name, setName] = useState('쩨리');
    const [birth, setBirth] = useState("2021년 2월 13일");
    const [code, setCode] = useState("말티즈");
    const [zender, setZender] = useState("남아");
    const [size, setSize] = useState("소형");
    const [weight, setWeight] = useState("3.6");
    const [active, setActive] = useState("많음");

    const [avatar,setAvatar] = useState('https://ifh.cc/g/QW45mB.png');
    const [grant, setGrant] = useState();
    const [isModalVisible, setModalVisible] = useState(false);

    const onChangeInput_name = (name) => {
        setName(name);
      };
    const onChangeInput_birth = (birth) => {
        setBirth(birth);
    };
    ...
    const onChangeInput_weight =(weight)=> {
        setWeight(weight)
    };
